...
> **Correction (2026-07-27): these runs trained with frozen biases.** A scratch-buffer overrun in the fused dbias kernel silently zeroed **every bias gradient in the model on every step** whenever the parameter dtype was bf16 — measured directly, all four bias tensors 0/27648, 0/9216, 0/36864, 0/9216 across six steps. fp32 was unaffected. The overrun landed exactly on the kernel's arrival counters, so no block ever finalized and `d_bias` was never written; and the repo's own fixtures could not see it, because at their B=4/T=64 shape the out-of-bounds writes deposit `0.0`, which is the counters' correct value. Fixed in `llmm/matmul.mojo`; the loss trajectory now diverges from the old behaviour by step 2, so this was changing training. The runs below are therefore GPT-2 without trainable biases. That is a legitimate architecture — several modern LLMs drop biases deliberately — and all six arms shared the defect, so the *precision comparison between them* stands. But the absolute numbers are not what the stated recipe describes, and the runs would need repeating to claim otherwise. See [`docs/ai/dbias_scratch_overrun_silent_zero_bug.md`](docs/ai/dbias_scratch_overrun_silent_zero_bug.md).
