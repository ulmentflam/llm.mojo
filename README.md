...
**fp8 training is quality-equivalent to bf16 at both scales, and NVFP4 pays a real but small cost.** At 124M bf16 finishes 0.0114 val loss ahead of fp8 (0.35%) and the HellaSwag gap is 10 items out of 10,042, a tie. At 774M the two are indistinguishable in a more interesting way: fp8 leads on HellaSwag by 42 items (37.67% vs 37.25%, z = 0.61, confidence intervals overlapping) while bf16 leads on val loss by 0.0018, and that val-loss ordering is decided by where the run stops. fp8 is ahead in 16 of the final 20 evaluations and the two curves cross in the last few hundred steps, so the final-step winner is the crossing point rather than a stable separation. Two metrics disagreeing in sign, over gaps smaller than the drift within a single run, is what noise looks like; I would not read either arm as better at 774M.

NVFP4 is the one arm that separates. It trails bf16 on val loss in 20 of the final 20 evaluations at both scales (+0.0291 at 124M, +0.0266 at 774M) and on HellaSwag at both, so unlike the fp8 gap the sign is consistent across metrics and scales. It is still a small cost for 4-bit forward GEMMs. Note the two low-precision arms are not equally aggressive: fp8 quantizes every per-block linear GEMM while the NVFP4 recipe quantizes only the middle blocks' MLP forward GEMMs.

Throughput on this box still favors bf16 at these model sizes; the quantize/scale overhead isn't amortized until larger GEMMs (see the scaling-sweep discussion under [Benchmarks](#benchmarks)). All six checkpoints are published in the [llm.mojo GPT-2 releases](https://huggingface.co/collections/ulmentflam/llmmojo-gpt-2-releases-6a51009ca7ef4e71b7ad7f2c) collection, each with safetensors plus the raw llm.mojo checkpoint.

An earlier version of these runs was retracted: a scratch-buffer overrun in the fused dbias kernel left some biases frozen, and not the same ones per arm, which confounded the comparison. All six arms above are the re-trained replacements, and every bias tensor in every arm is fully trained. See [`docs/ai/v1_precision_table_retraction.md`](docs/ai/v1_precision_table_retraction.md).

!['Precision comparison: val loss'](figures/precision_valloss_2026-08-13_NVIDIA-RTX-PRO-6000-Blackwell-Max-Q-Workstation-Edition.png)

!['Precision comparison: HellaSwag'](figures/precision_hellaswag_2026-08-13_NVIDIA-RTX-PRO-6000-Blackwell-Max-Q-Workstation-Edition.png)

Reproduce with `make figure-precision` (or `make figures` for every data-derived figure). It renders from `docs/ai/v2_arm_results/*.json`, the same records that generate the table above, and writes a `.json` sidecar next to each `.png` carrying the source data verbatim.
