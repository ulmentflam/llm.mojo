...
path needs only a size reserve (§7.3), not a rewrite.
...
inference reserve of §7.3, exactly**. At K=64 the vocabulary slice
...
## 7. Three notes that cost real time
...
### 7.2 A status signal that could never fire

This one is not about cross-entropy at all. It is recorded because it
generalizes, and because it very nearly ate this work's final gate.

The gate run had to wait for a box-wide lock held by another team, so it was
watched by a poll loop whose completion test was:

```bash
if grep -q "tests run:" "$LOG" && ! pgrep -f "run_gate.sh"; then
    echo "FINISHED"; break
fi
```

The string `run_gate.sh` appears **in the watcher's own command line**. So
`pgrep -f "run_gate.sh"` always matched — the watcher itself — and `! pgrep`
was permanently false. The loop could report the early stages and would then go
quiet forever.

**And "quiet forever" is indistinguishable from "still running."** That is the
whole problem. A watcher that crashes loudly gets fixed in a minute. A watcher
that reports the first two of three stages and then falls silent looks exactly
like a long-running job, and the longer you wait the more reasonable waiting
seems. It was caught here only by reading the process table and noticing the
watcher's own PID in what it was supposed to be watching.

It belongs to a family this campaign hit more than once: **a status signal
produced by something other than an independent observation of the world.** The
sibling cases were a stop script that printed "clean exit on SIGINT" after
signalling a process group that no longer existed, and a gate that reported
success without having executed. In each, the signal was generated by the same
process whose behaviour it claimed to describe.

Two rules fall out, and they cost nothing:

1. **Have the work itself announce completion, not an observer inferring it.**
   The fix here was one line appended to the job — `touch "$S/gate_DONE"` — and
   a watcher that tests for that file. The sentinel exists only if the job
   actually reached its end. No pattern matching, nothing self-referential.
2. **Ask of every watcher: if the thing I am watching died right now, would
   this emit anything?** If the answer is no, the watcher reports success and
   silence with the same signal, and is worse than no watcher — because it
   converts an unknown into a false reassurance.

The same failure shape is why §9.4 distinguishes gates that were *run* from
gates that were *reasoned about*.

### 7.3 Inference still wants the whole row
...
### 9.4 Gates: what was run, and what was not

The distinction below is deliberate. §7.2 is about status signals that describe
something other than an observation of the world; a gate summary is exactly such
a signal, so it says which gates were *executed* and which were *reasoned about*.

**Executed, green:**

| gate | result |
|---|---|
| `make format` | clean, zero residual diff |
| `make lint` | passed (also as the pre-commit hook on every commit here) |
| `make check` | exit 0 (lint + `build-mojo` + `build` + `build-profile`) |
| build, flag OFF | zero compiler diagnostics |
| build, flag ON | zero compiler diagnostics |
| `tests/test_chunked_cross_entropy.mojo` | 10/10 |
| `tests/test_lm_head_vocab_tiling.mojo` | 10/10 (regression check on the `llmm/matmul.mojo` edit) |
| `test_gpt2.mojo gpu`, flag OFF | LOGITS OK, LOSS OK, 16/16 tensors |
| `test_gpt2.mojo gpu`, flag ON | LOGITS OK, LOSS OK, 16/16 tensors (§9.1) |

**Not executed by this workstream: the full `make test`.** The box lock was held
by another team's suite for the whole window. Rather than sit in a two-hour lock
queue to gate one branch, the coordinator took it over to run once across the
merged tree — which is strictly better, since what matters is the merged result.
**So: `make test` is unverified here and must be green on the merge before this
lands.** Unrun components are `tests/test_zero*.mojo`, `tests/test_encoder_row_sparse.mojo`,
and the Python suite. Note also that the `test_gpt2.mojo` runs above were made
*before* `main` was merged, so they predate the corrected `expected_losses`; they
are cited only for the tensor/logits/loss checks, which are unaffected.

**Reasoned about, not measured** — stated as reasoning, not as a result: with
the flag off every chunked path is `comptime`-dead, so the default build should
be behaviourally identical to `main`. The one edit that is *not* flag-gated is
the packed fast path in the two tile GEMMs, which engages only when the caller's
leading dimension equals the tile width — never true on the default path (V_p =
50304 against a 6400-wide tile) nor in phase 1's suite. The direct evidence for
that specific claim is phase 1's suite passing 10/10; the rest is an argument,
and the merge gate is what will actually test it.

One coverage gap for whoever maintains the smoke subset:
...
State at stand-down, for whoever picks this up.

**Branch** `zero/lmhead`, worktree `/home/evan/workspace/wt-lmhead`. Committed
locally, **never pushed**; the coordinator merges.

| SHA | what |
|---|---|
| `0957fa5` | the implementation, tests, and this document |
| `fbc0cc4` | record the then-outstanding `make test` gate |
| `73f6943` | doc corrections: verify-gpu resolution, phase-1 obstacle overturned, the `fused_classifier_cpu` callout, ar5iv citation links |
| `41970ff` | MB/MiB conflation fix and the unit convention |
| `15e056a` | merge of `main` (which carries the corrected `expected_losses`) |

Working tree clean. Nothing uncommitted.

**The flag.** `-D LLMM_LM_HEAD_CHUNKED_CE=1`, **default 0 = today's behaviour**.
Composes with `-D LLMM_LM_HEAD_VOCAB_TILES=K` (default 8); a no-op at K=1.

**Measured, B=32 T=1024 fp32, one RTX PRO 6000 Blackwell:**

| | OFF | ON, K=8 | ON, K=64 |
|---|---:|---:|---:|
| `logits` | 6288.0 MiB | 800.0 MiB | 196.5 MiB |
| process total | 71.42 GiB | 66.06 GiB | 65.47 GiB |
| step time | 431.11 / 436.64 ms | 457.54 / 463.08 ms | 464 ms |

Overhead **+26.4 ms, +6.1%**, reproducing to 0.01 ms across two paired
repetitions. Exchange rate **5.36 GiB per 26.4 ms**, ~0.88 GiB per 1% of step
time.

**The one thing left to do:** `make test` on the merged tree (§9.4).

**Open items, in priority order:**

1. **`make test` on the merge.** The only unrun gate.
2. **The 196.5 MiB floor is removable.** It exists only because generation reads
   `acts.logits + row·V_p` and so needs a full-width row (§7.3). Generation
   actually wants *one* row. Teaching the sampler to read a dedicated V_p-sized
   row buffer would drop the floor to the tile size (117 MiB at K=64) and, more
   usefully, make the saving independent of `max_seq_len`. Deliberately not done
   here: it touches `train_gpt2.mojo` and `infer_gpt2.mojo`, and `infer_gpt2` is
   doubly ungated (phase 1's finding — `make check` never builds it, and its
   smoke tests skip on a missing checkpoint).
3. **`fused_classifier_cpu`'s misaligned-load segfault** (§7.1). Latent, not
   reachable from the trainer today, will bite a non-standard vocabulary.
4. **`att_probs` is now 24× `logits`** — 18.0 GiB against 800 MiB at K=8. After
   this change the logits tensor is not even the second-largest activation
   (`fch_gelu`, 4.5 GiB, is). Out of scope here and owned elsewhere, but it is
   where the next multi-GiB win is.

**Gotchas that cost real time, so they do not cost it twice:**

- **You cannot unit-test `fused_classifier_cpu` at a small `V_p`.** It segfaults
  (§7.1). Use float64 references, as `tests/test_chunked_cross_entropy.mojo`
  does throughout.
- **Assert the realized tile count inside the test.** Nothing else in the repo
  drives this path above `OC=3072` while production runs at 50304, so a suite
  can go green having exercised a single tile. Every test here asserts the count.
- **K is a request, not a count** (phase 1's finding, unchanged): widths round up
  to a multiple of 128, so K=32→31 tiles, K=64→57, K=128→99.
- **Do not reuse `softmax_phase_1_and_2_*` on a tile.** Its aligned loads assume
  a leading dimension that is a multiple of the SIMD width; a tile's is
  arbitrary. This is the same root cause as the item above.
- **`allocate_gradients()` runs before `batch_size` is set**, so pool sizing
  cannot read B/T. `_logits_elems()` is called from `allocate_activations()`,
  which *can*.
- **Watchers must be checked against the failure case, not the success case**
  (§7.2).
