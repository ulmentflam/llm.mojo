...
Forward (\texttt{crossentropy\_ohe\_fwd}): given probabilities
$p_{bt} \in \R^{V_p}$ and a target token $y_{bt} \in \{0,\dots,V-1\}$,
\begin{equation}
...
\begin{derivation}
  \TODO{Differentiate \eqref{eq:ce-fwd} w.r.t.\ $p_{bt,i}$; note which
    entries of $\dL{p_{bt}}$ are nonzero and compare against the
    assign-only-the-target-column behavior of \texttt{crossentropy\_ohe\_bwd}.}
\end{derivation}
