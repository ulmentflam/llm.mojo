...
def test_single_element() raises:
    ...
    logits[unsafe_offset=0] = Scalar[DType.float32](3.5)
...
def test_uniform_logits() raises:
    ...
    for i in range(n):
        logits[unsafe_offset=i] = Scalar[DType.float32](0.0)
    for k in range(20):
...
def test_peaked_distribution() raises:
    var n = 5
    var logits = _alloc_logits(n)
    logits[unsafe_offset=0] = Scalar[DType.float32](-1.0)
    logits[unsafe_offset=1] = Scalar[DType.float32](0.0)
    logits[unsafe_offset=2] = Scalar[DType.float32](10.0)
    logits[unsafe_offset=3] = Scalar[DType.float32](-2.0)
    logits[unsafe_offset=4] = Scalar[DType.float32](-3.0)
    for k in range(50):
...
def test_arithmetic_progression() raises:
    ...
    var logits = _alloc_logits(n)
    for i in range(n):
        logits[unsafe_offset=i] = Scalar[DType.float32](Float32(i) * 0.25 - 1.0)
    for k in range(100):
...
def test_extreme_logits() raises:
    var n = 6
    var logits = _alloc_logits(n)
    logits[unsafe_offset=0] = Scalar[DType.float32](-100.0)
    logits[unsafe_offset=1] = Scalar[DType.float32](-50.0)
    logits[unsafe_offset=2] = Scalar[DType.float32](0.0)
    logits[unsafe_offset=3] = Scalar[DType.float32](50.0)
    logits[unsafe_offset=4] = Scalar[DType.float32](100.0)
    logits[unsafe_offset=5] = Scalar[DType.float32](-100.0)
    for k in range(30):
