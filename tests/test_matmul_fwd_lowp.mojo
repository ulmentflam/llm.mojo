...
def test_input_amax_steady_state_ignores_current_spike_gpu() raises:
    ...
    var host_w = ctx.enqueue_create_host_buffer[DT](n_w)
    for i in range(n_w):
        host_w.unsafe_ptr()[unsafe_offset=i] = Float32(0.015625).cast[
            DT
        ]()  # 2^-6, exact
    var host_b = ctx.enqueue_create_host_buffer[DT](out_channels)
    for i in range(out_channels):
        host_b.unsafe_ptr()[unsafe_offset=i] = Float32(0.0).cast[DT]()
    ...
    for i in range(H):
        var v = Float32(1 << i)
        for j in range(n_in):
            host_in.unsafe_ptr()[unsafe_offset=j] = v.cast[DT]()
        dev_in.enqueue_copy_from(host_in)
        ctx.synchronize()
        matmul_fwd_lowp[DT, "gpu", use_gelu=False, has_bias=True](
    ...
    for j in range(n_in):
        host_in.unsafe_ptr()[unsafe_offset=j] = spike.cast[DT]()
    dev_in.enqueue_copy_from(host_in)
    ctx.synchronize()
    matmul_fwd_lowp[DT, "gpu", use_gelu=False, has_bias=True](
        dev_out.unsafe_ptr().as_unsafe_any_origin(),
