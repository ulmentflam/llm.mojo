...
struct GPT2[target: StaticString, WORLD_SIZE: Int = 1, recompute: Bool = False]:
    ...
    def forward(
        ...
        if targets != NULL_INT32_PTR:
            ...
            # read SYNCHRONOUSLY on the host: fused_classifier_cpu runs under
            # sync_parallelize and never enters the device queue, so nothing
            # orders it after the copy. Without this barrier the reader races
            # the copy, and any row whose `d_losses_ptr[idx]` read wins sees the
            # buffer's PRE-copy contents — zero, or uninitialized heap on a
            # freshly allocated model. That row's dlogits are then garbage, and
            # since dlogits is the root of the whole backward, EVERY parameter
            # gradient is corrupted. The loss is unaffected (it is computed from
            # the logits before the in-place dlogits overwrite), which is what
            # made this so hard to see: ~1 in 4000 CPU steps, silently wrong
            # gradients.
            #
            # Placed BEFORE the chunked/dense branch on purpose. The dense arm
            # reads grad_acts.losses right below; the chunked arm defers that
            # read to backward's pass 2. synchronize() drains the queue, so one
            # barrier here makes the seed visible to BOTH, whenever they get to
            # it — rather than leaving the chunked path to re-derive the same
            # ordering requirement somewhere less obvious.
            #
            # GPU targets need no barrier — there the copy and the consuming
            # kernel are both enqueued on this context's stream, which already
            # orders them.
            comptime if is_cpu[Self.target]():
                self.ctx.synchronize()
